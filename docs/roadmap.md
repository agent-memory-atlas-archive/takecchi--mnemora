...
#### ⚠ ただし「誰が呼ぶか」で費用が変わる —— (甲) と (乙) を分けること

**2026-09-16 訂正: 本節は当初「項目4・項目5 は配線しないことが記録済みの判断であり、先に ADR が要る」と*無条件に*書いていた。⟹ 誤りだった。**成り立つのは **(甲) を採るときだけ**である。
...
- [ADR 0114](./decisions/0114-archive-sweep-for-decayed-memories.md) 採らなかった案5 「**`Runtime.tick`/`Runtime.observe` から自動的に掃引を走らせる。**却下。北極星の問い2 …に照らすと、掃引を呼ばない運用でも observe/recall は成立しなければならない」
- [ADR 0134](./decisions/0134-mark-contested-explicit-operation.md) 決定1 が自動検出（抽出時・背景処理）を「**LLM を使わない一般化の方法が無い**」として見送っている

**⛔ しかし、同じ ADR 群が (乙) を*名指しで指定している*（逐語）:**

- ADR 0114 **決定3**: 「**呼び出し側が明示的に `sweepArchive` を呼んだときだけ走る保守操作である**」
- ADR 0134 **決定2**: 「**呼び出し側（人・上位のアプリケーション層・将来の自動検出）**が既に『この2件は対向する』と決めていることを前提に、…機械的に書き込むだけである」
- ADR 0150 **決定2**（解決側）: 「**この口も「どちらが正しいか」を判定しない。**決着は**呼び出し側が渡す**（`{ kind: 'supersede', winnerId }` か `{ kind: 'both_active' }`）」

⟹ **`examples/chat` が呼ぶのは「上位のアプリケーション層」そのものであり、上書きではなく ADR に*沿った*使い方である。**

**⚠ `mark-contested.test.ts` が固定しているのは (甲) の不在だけである。**アサーションの現物は「`runtime.tick()` を呼んでも `active` な Memory は `contested` にならない」1本であり（`describe("runtime.markContested — tick()/observe() から呼ばれない")`）、**アプリ側が明示的に `markContested` を呼ぶことは禁じていない**——直後の `describe` は、明示的に呼んだときに段3が実際に発火することを測っている。

**北極星の問い2 に (乙) を当てても落ちない。**[ADR 0134](./decisions/0134-mark-contested-explicit-operation.md) が既にこれを実施しており、逐語で「**成立する。**`markContested` は明示的な呼び出しでしか動かない。…呼ばなければ、今日と全く同じ挙動が続く」と結論している。

**⭐ そして (乙) は既に実践されている。**`examples/chat` の `archive-sweep-cost` サブコマンド（`examples/chat/src/archive-sweep-cost.ts:290`）が `runtime.sweepArchive()` を実際に呼んでおり、**ADR の上書きを一度も必要としていない。**

**⟹ 残る費用は「ADR の上書き」ではなく、項目5 について「`examples/chat` が*どうやって*矛盾だと決めるか（利用者の訂正発話か、LLM か）を選ぶこと」である。**ADR 0134 はこれを意図的に呼び出し側へ残した——**ただしそれはアプリ側の設計選択であって、記録済みの判断の上書きではない。**

#### ⟹ §7.2 の予定が費用を数え落とすのは、(甲) を採るときだけ

**⚠ (甲) を採るなら、§7.2 はその費用（ADR の上書き）を数え落としている**——§7.6 の延びる条件にも入っていない。⛔ **この警告は消さずに残す。**将来 (甲) を検討する人が、同じ調査をゼロからやり直さずに済むようにするためである。

**⭕ (乙) を採るなら、追加の ADR は要らない。**[ADR 0134](./decisions/0134-mark-contested-explicit-operation.md) 決定2 が呼び出し側を明示的に許しているため、§7.2 の見積もりはこの点については欠けていない。
