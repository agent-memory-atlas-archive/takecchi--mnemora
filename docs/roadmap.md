...
- [ADR 0150](./decisions/0150-resolve-contested-explicit-operation.md) **決定2**（解決側）: 「**この口も「どちらが正しいか」を判定しない。**決着は**呼び出し側が渡す**（`{ kind: 'supersede', winnerId }` か `{ kind: 'both_active' }`）」

⟹ **`examples/chat` が呼ぶのは「上位のアプリケーション層」そのものであり、上書きではなく ADR に*沿った*使い方である。**

**⚠ `packages/core/src/__tests__/mark-contested.test.ts` が固定しているのは (甲) の不在だけである。**アサーションの現物は「`runtime.tick()` を呼んでも `active` な Memory は `contested` にならない」1本であり、**アプリ側が明示的に `markContested` を呼ぶことは禁じていない**——直後の `describe` は、明示的に呼んだときに段3が実際に発火することを測っている。⟹ **呼び出し元を禁じる歯は、ソースを走査しても存在しない。**
